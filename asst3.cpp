    ...
    void update() override {
        ...
        for(int i = 0; i < 16; i++) {
            screen->drawLine(Vec2D(50, 50), Vec2D(50, 50) +
                                            Vec2D(cos(2.0*Consts::PI*i/16.0), sin(2.0*Consts::PI*i/16.0))*30, Consts::WHITE_COLORS[i%5], 3);
        }
        */
        // Testing drawing Triangle:
        //std::array<Color, 3> colors{Color{255, 0, 0}, Color{0, 255, 0}, Color{0, 0, 255}};
        /*
        Triangle triangle1{{Vec4D{10, 10}, Vec4D{10, 160}, Vec4D{160, 10}},
                           {Vec2D{0, 0}, Vec2D{0, 1}, Vec2D{1, 0}}};
        Triangle triangle2{{Vec4D{10, 160}, Vec4D{160, 10}, Vec4D{160, 160}},
                           {Vec2D{0, 1}, Vec2D{1, 0}, Vec2D{1, 1}}};

        screen->drawTriangle(triangle1, Image("obj/test2.png"));
        screen->drawTriangle(triangle2, Image("obj/test2.png"));
        */
        //world->object(ObjectNameTag("house"))->rotateUp(Time::deltaTime()/2);

        screen->setTitle(std::to_string(Time::fps()));
